...
def is_sorted(arr)
  (0...arr.length - 1).each do |i| # loop through the entire array, except the last number...
    if arr[i] > arr[i + 1]         # which will be checked by the second to last index
      return false                 # if a switch needs to happen, it's not sorted
    end
  end
  true
end
