...
# In this tutorial, we will discuss a hypothetical app with Post and Tag models. 
# When users create a Post via the new post form, we want them to be able to tag 
# their posts with existing tags. We can accomplish this with a checkbox input 
# field on our post form where users can check all of the tags that they want a 
...
# Remember that Rails 5 automatically validates presence of belongs_to 
# associations. But let's say you were working in an older codebase using 
# Rails 4, which doesn't share this behavior. When validating the presence of 
# Tagging fields, we would have to validate the presence of post and tag rather 
# than post_id and tag_id. Why? Ultimately, we want to be able to create a 
# Tagging at the same time we create a new Post. Since our post won't be saved 
# to the database yet, it won't have an id and validating post_id would fail. 
# We validate the existence of a post instead.

# Also, notice that in our associations for taggings, we use dependent: :destroy 
# and inverse_of. The dependent: :destroy assures that we destroy taggings when 
# we destroy their associated Post. The inverse_of allows us to find the 
# associated Tag or Post for a tagging in memory, even if it has not yet been 
# saved to the database. (Otherwise, it would search in the database for a post 
# where tagging.post_id == post.id, and we don't have an id assigned yet!)

### Tagging Posts with Post#tag_ids= ###

# Say we had a Post and wanted to tag it with several Tags. We would need to 
# create a Tagging for each unique Post/Tag combo.

# When we define our has_many :taggings and has_many :tags, through :taggings 
# on Post, Rails provides us with a Post#tag_ids= method that takes in an array 
# of tag_ids.

# This method does a lot more than meets the eye. It:

# a. Creates a new Tagging for the post for each new tag_id.
# b. Destroys existing taggings for that post that are not in the tag_ids array.
...
# and 4 and deletes any Taggings for Tags whose ids are not in the array.

# We could also pass in an array of tag ids when we create a new post like such:

Post.create(text: 'new post', tag_ids: [1,2,3,4])

# This will simultaneously create a new post along with its associated taggings 
# in a transaction: If any record fails to save to the database, the entire 
# transaction will be rolled back.

### Controller ###

# Now that we have our model set up, let's see how we'd a handle a POST #create 
...
# tag_ids as a permitted param, since we'll be passing an array of tag_ids from 
# our form. We also must specify that tag_ids permits an array - otherwise, 
...
# This just makes sure that, even if we uncheck every box, a param is sent 
