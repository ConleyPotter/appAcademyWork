...
// When running JavaScript code in the browser, the browser provides an API by 
// which JavaScript code can access the HTML content of a page. This API is 
// called the Document Object Model. The DOM API is standardized by the World 
// Wide Web Consortium (W3C); all major browsers implement it, though there are 
...
// When the script tag is run, document.getElementById("cats") asks the browser 
// to find an element with the id cats. This method returns a subclass of 
// HTMLElement, which inherits from Element.

// We then build a new li element, via document.createElement. We set its 
// textContent, and then append this to the ul. The new element is not inserted 
// into the page unless we call appendChild.

// In sum: after 1 sec, an li saying "This is injected by JavaScript"
// appears.
...
// From JavaScript, we have complete freedom to modify the DOM: insert elements, 
// remove them, change attributes... But this dynamic modification of the HTML 
// document would be pointless without event handling.
...
// Here we used the EventTarget#addEventListener method to ask the browser to 
// store a function. EventTarget is a parent class of HTMLElement. The browser 
// will call the function when a submit event happens to the form.

// When the user clicks on the submit button in the form, the browser will 
// trigger (also called fire) an event. Specifically, it will trigger a submit 
// event on the form element.

// When this happens, the browser will see that the JavaScript code had 
// previously asked it to run a function whenever a submit event happens to this 
// form. The browser will stop what it is doing, and call the JavaScript code.
...
// MDN, as ever, has a full list of DOM Events. {https://developer.mozilla.org/en-US/docs/Web/Events}
