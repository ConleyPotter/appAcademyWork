// Functions that operate on other functions, either by receiving them as
// arguments or returning them, are called higher-order functions. Functions
// that are passed as parameters to and invoked in a higher-order function
// are known as callbacks. This perspective makes it especially easy to
// abstract over the actions they represent.

// CLOSURES //

// A closure, also known as lexical scoping, is a function that uses free
// variables, variables defined outside of its scope. Closures tend to come
// in handy when writing higher-order functions. Consider the following code:
...
// value would not work if the inner function could not close over operationCb,
// a variable defined outside of its scope.

// COMPOSING FUNCTIONS //

// Composing functions is an idea that you were probably first introduced to in
// math class. A function that composes two functions might look like the
// following:
...
// This pattern applies to more than just mathematical operations, of course
// -- we can use this pattern more generally.
...
// collects these arguments until their count reaches that n. At which point, 
// it calls the original function. This pattern allows us to write functions 
// that can take arguments as they become available and, in the meantime, 
// be passed around among other functions.
...
// automatically bound to the context (this) that existed when they were 
