...
def pow(base, exponent)
  ...
  sum = 0
  (0...base).each { |el| sum += pow(base, exponent - 1) }
  sum
end
...
def lucas_number(n)
  ...
  return 1 if n === 1
  lucas_numbers = []
  (0..n).each { |el| lucas_numbers << lucas_number(el - 1) + lucas_number(el - 2) }
  lucas_numbers.last
end
...
def flatten(data, result=[])
  if tmp = Array.try_convert(data)
    data.each do |el| 
      if tmp = Array.try_convert(el)
        el.each { |inner_el| flatten(inner_el, result) }
      else
        result << el
      end
    end
