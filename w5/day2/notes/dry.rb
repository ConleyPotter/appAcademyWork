...
# In The Pragmatic Programmer, Hunt and Thomas define the DRY principle as follows:
...
# See how we've repeated the bit that checks that the zip code is valid? This 
# is bad; if there were a bug with our zip code validation, we'd have to fix 
# the code in two places. This is a pain, and we'll probably forget to fix the 
# duplicated code everywhere, so it's also a source of bugs. For this reason, 
# we want to find a way to eliminate the duplicated code:
