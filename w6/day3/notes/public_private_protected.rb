...
# Note that as public is the default access level for methods, so in the example 
# from the last section we didn't have to specify that #meow was public.

### private ###

    # - private methods cannot be called with an explicit receiver. Because you 
    # cannot specify an object when using them, private methods can be 
    # called only in the defining class. ...
...
# This differs somewhat from languages like C++/Java, where private methods are 
...
# So protected methods can be called with an explicit receiver, so long as
# the caller is of the same class.
...
# This way members of the Dog class can access other dominance scores, but they 
# are secret to everyone outside the Dog class.

### Access controls are not about security ###

# Note that these access modifiers are not for security. In fact, they're super 
# easy to subvert. Check it out:
...
# We'll cover #send another day; but for now you should know that it allows 
# you to pass in a symbol (or string) and call a method with that name. 
# In particular, it ignores privacy levels.

# Instead of security, you should be using access controls to describe to other 
# programmers reading your code:

    # - What methods are the "interface" that they'll want to use, and what are 
    # underlying details they may wish to ignore.
    
    # - What methods are "supported" and public, and which ones are liable to 
    # change. Private methods, because they usually are focused on internal 
    # details, often are removed or changed as the code grows. There is 
    # typically a greater effort to continue to support and not break the 
