...
# To test a class, you will often want to instantiate an instance of the object 
# to test it out. In this case, you may want to define a subject for your tests.
...
# fails. In the first test, we expect that the position is [0, 0]. In the 
# second test we move the robot, and then expect the position to have changed.
...
# let defines a method (e.g. light_item, maxx_weight_item) that runs the 
...
# block to declare objects accessible to specs, but we'll avoid defining instance 
# variables in specs. Always prefer let. Here's a SO post that clearly describes 
# why that is. {http://stackoverflow.com/questions/5359558/when-to-use-rspec-let}

# Here's a blog post with some nice examples using let - note how the author 
# uses it in conjunction with subject (some fancy and clean stuff).
...
# scope. Since every it is a different scope, let does not persist state between 
# those specs.
