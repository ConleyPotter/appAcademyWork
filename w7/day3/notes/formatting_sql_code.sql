...
-- Different programmers use different SQL conventions, but in preparation 
-- for ActiveRecord and Rails, which have their own conventions, you should:

--  - Always name SQL tables snake_case and pluralized. (e.g., 
--    musical_instruments, favorite_cats)
--  - If a musician belongs to a band, your musicians table will need to stores 
--    a foreign key that refers to the id column in the bands table. The foreign 
--    key column should be named band_id.
...
-- Notice that each component of the SQL statement starts with the keyword 
-- aligned left. The body of each component is indented two spaces. Complex 
-- WHERE clauses are parenthesized and indented two spaces on the following line.
...
-- I put the leading paren on the prior line, indent the query two spaces, and close
-- with a trailing paren at the start of a new line. I put the ON of a JOIN right 
