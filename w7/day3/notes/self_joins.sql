...
-- itself. The way you should visualize a self join for a given table is by imagining 
-- a join performed between two identical copies of that table.
...
-- different tables (presumably with two different names), which made it easy 
-- to reference a specific column in a table. Since we only deal with one table 
-- in a self join, we have to use aliases.
...
-- between the duplicates of the same table to JOIN them. Keep in mind, the 
-- keyword AS is not necessary to alias tables or columns. The above SQL query 
-- could be rewritten:
