...
# An implementation of SQL is the software that actually processess your SQL 
# commands. A synonymous term is RDBMS (relational database management systems): 
# this is the software that stores the data, and later figures out how to fetch 
# it back when queried.

# Each implementation is different; they all speak SQL, so there is a mostly 
# common interface to all the systems (each system has its own quirks about SQL 
# syntax). However, the various implementations also can differ significantly 
# under-the-hood. Each has its own specialties and pitfalls.
...
# SQLite3 is another SQL implementation; it is a very simple, public-domain 
# implementation. Whereas most database servers require a lot of setup and 
# configuration, SQLite3 is serverless. There is no separate server process that 
# waits to receive queries from your application. Instead, sqlite3 is a library 
# loaded by your program, which then directly interacts with the underlying 
# database data. This makes it easy to configure sqlite3 (there is no 
...
# very sophisticated query planner to figure out how to execute your query in 
# the best way; this can result in poor performance when working with larger 
# sets of data. However, the simplicity of setting up SQLite3 makes it very
# common as a development database used while developers are building an app, or 
# as an embedded database distributed as part of another program.

# SQLite3 is the most widely deployed database of any. Every copy of Firefox
# embeds a SQLite3 database (your user preferences are stored in it). It is 
# behind every iOS app.

### A Superquick Demo ###

# First we write a SQL source file to generate our data. Check it out here. 
# Read this demo and the comments!

# Then we can use SQLite3 to run the sql source, generating the tables.

# ~/sql-demo$ cat create_tables.sql | sqlite3 school.db

# This tells sqlite3 to read/store data in school.db; since this doesn't exist 
# yet, sqlite3 will create it. The | (pipe) connects the output of the command 
# on the left (cat create_tables.sql) to the input of the command on the right 
# sqlite3 school.db). We say that create_tables.sql is "piped into" the sqlite3 
# interpreter.
...
# user interaction), we either use the SQLite3 client, or, if the changes should 
# be made by a program and not a person, the sqlite3 Ruby gem.
...
# PRIMARY KEY, SQLite3 will take care of populating it automatically for me.
...
# SQLite has a few special "meta" commands .tables and .schema which list the 
# existing tables and their schemas. Here I insert a new department into the 
# table, and then select it back out.
...
# To interact with SQLite3 from your Ruby programs, use the gem sqlite3-ruby. 
# I've written a demo (file: ./school.rb) to show you how to use the gem. It is a 
# very simple, thin wrapper over SQL. You may need to install the sqlite3 gem:
...
# There's a good FAQ to check out.{http://www.sqlite.org/faq.html}
...
# How to drop columns (hint: it's ugly).
