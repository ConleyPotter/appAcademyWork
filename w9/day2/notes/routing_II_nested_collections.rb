...
# We can restrict what routes we build for a resource using the :only option. 
# Here, we tell the nested resource to only generate the :index route.

# If we didn't specifically restrict the routes, Rails would generate all the 
...
# /magazines/42/articles/101 would both route to the same Article. One of these 
# is superfluous.

# Also, the /magazines/42 bit of the /magazines/42/articles/101 path is 
# redundant. ArticlesController#show doesn't need the magazine id to find the 
# article; it can just use the article id directly. If we wish to use the 
# magazine_id, we can always look it up from the article id:

Article.find(id).magazine_id

# As a general rule, never generate any of the member routes when nesting. 
# Member routes should only belong to top level resources. There's nothing wrong 
# with defining the same resource at two levels.
...
# I like nested index routes. 
# This should trigger an index action to get all the articles belonging to the 
# given magazine. Note that this is different from the top level /articles index 
# route; that one should return all the articles in the system. Because it 
# returns a subset, /magazines/:magazine_id/articles does not repeat the top 
# level index route in the way nested member routes do.

# I don't like nested create routes. 
# create is superfluous like the member routes. It doesn't do anything 
# differently from the top-level route. If you want to create a new Article for 
# Magazine 123, you don't need to specify 123 in the create URL. Instead, you 
# can use a hidden field to upload the magazine_id via the HTTP POST body. While 
# you might think that you would prefer to encode the magazine_id in the URL 
# rather than a hidden field, we will see that having non-nested create routes 
# will make it simpler to define our React components later on. Start practicing 
# this pattern now, and always create through a top-level route.

# A nested new route is okay. /magazines/123/articles/new can present a form for 
# an Article for Magazine #123. It should still post to /articles, though.